...
class OnboardingHonorariesTestCase(unittest.TestCase):
    ...
    def test_onboarding_salary_test_case(self):
        ...
        xpath_string = "//div[@id='1.1.3.4']/div/div/label/span"
        while not self.is_element_present_by_xpath(xpath_string):
            pass
        wait.until(lambda driver : not driver.find_element_by_xpath(xpath_string).text is None)
        self.assertTrue(u"¿Prestaste estos servicios por fuera de Colombia" in driver.find_element_by_xpath(xpath_string).text)
        self.assertTrue(u"¿Realizaste aportes a seguridad social sobre estos ingresos" in driver.find_element_by_xpath("//div[@id='1.1.3.7']/div/div/label/span").text)
        self.assertTrue(u"¿Incurriste en costos o gastos necesarios para prestar estos servicios" in driver.find_element_by_xpath("//div[@id='1.1.3.8']/div/div/label").text)
