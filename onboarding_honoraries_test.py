...
class OnboardingHonorariesTestCase(unittest.TestCase):
    ...
    def test_onboarding_salary_test_case(self):
        ...
        xpath_string = "//div[@id='1.1.2.4']/div/div/label/span"
        while not self.is_element_present_by_xpath(xpath_string):
            pass
        wait.until(lambda driver : not driver.find_element_by_xpath(xpath_string).text is None)
        self.assertTrue(u"¿Recibiste estos honorarios por actividades realizadas fuera de Colombia" in driver.find_element_by_xpath(xpath_string).text)
        self.assertTrue(u"Realizaste aportes a seguridad social sobre estos ingresos" in driver.find_element_by_xpath("//div[@id='1.1.2.7']/div/div/label/span").text)
        self.assertTrue(u"¿En este trabajo participaste en un proyecto avalado por Colciencias" in driver.find_element_by_xpath("//div[@id='1.1.2.8']/div/div/label").text)
        self.assertTrue(u"¿Para la realización del trabajo, contrataste a dos o más personas por más de 90 días" in driver.find_element_by_xpath("//div[@id='1.1.2.9']/div/div/label").text)
