...
class OnboardingNegociosTestCase(unittest.TestCase):
    ...
    def test_onboarding_salary_test_case(self):
        ...
        xpath_string = "//div[@id='1.1.4.27']/div/div/label/span"
        while not self.is_element_present_by_xpath(xpath_string):
            pass
        wait.until(lambda driver : not driver.find_element_by_xpath(xpath_string).text is None)
        self.assertTrue(u"¿Tuviste una deuda directamente asociada a la operación del negocio durante el año 2017" in driver.find_element_by_xpath(xpath_string).text)
        self.assertTrue(u"¿Este negocio desarrolla su actividad fuera de Colombia" in driver.find_element_by_xpath("//div[@id='1.1.4.28']/div/div/label/span").text)
